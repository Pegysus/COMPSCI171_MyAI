...
class MyAI( AI ):
	...
	# TODO: change frontier search algorithm to look for a set # of uncovered nodes
	# Using those nodes, develop the covered frontier nodes that are at play
	# (limit size before searching to increase efficiency + eliminate confusion in model checking)
	...
	def _cov_frontier_search(self, c, r):
		for dc, dr in self._dir:
			if self._inbound(c+dc, r+dr) and self._grid[c+dc][r+dr].get_unc() == GLOBAL_UNDEF \
					and (c+dc, r+dr) not in self._cov_f and len(self._cov_f) < self._max_model_length:
